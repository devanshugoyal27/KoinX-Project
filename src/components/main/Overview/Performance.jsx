...
const Performance = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      console.log('per', response.data[0]);
    } catch (error) {
      setError(error);
    }
  };
      ...
      <div className="bg-white p-4 mt-5 rounded-md">
        ...
        <div className="flex justify-between items-center gap-8">
          <div className="text-center">
            <p className="text-sm text-gray-600 font-semibold">Today's Low</p>
            <p className="text-xl">{data?.low_24h.toLocaleString("en-IN")}</p>
          </div>

          <div className="h-2 w-[70%] customBg rounded-md relative">
            <p
              className="absolute flex items-center flex-col right-0"
          ...
          <div className="text-center">
            <p className="text-sm text-gray-600 font-semibold">Today's High</p>
            <p className="text-xl">{data?.high_24h.toLocaleString("en-IN")}</p>
          </div>
        </div>
        ...
        <div className="flex justify-between items-center gap-8 mt-10">
          <div className="text-center">
            <p className="text-sm text-gray-600 font-semibold">52W Low</p>
            <p className="text-xl">16,930</p>
          </div>

          <div className="h-2 w-[70%] customBg rounded-md relative"></div>
          <div className="text-center">
            <p className="text-sm text-gray-600 font-semibold">52W High</p>
            <p className="text-xl">{data?.ath.toLocaleString("en-IN")}</p>
          </div>
        </div>
